...
| E-08 | Callbacks 覆盖五个固定时点；流返回后的读取错误不会进入 `OnError`，流副本必须关闭 | 已验证 | [`callbacks/doc.go`](https://github.com/cloudwego/eino/blob/v0.9.12/callbacks/doc.go#L17)；[`callbacks/interface.go`](https://github.com/cloudwego/eino/blob/v0.9.12/callbacks/interface.go#L62)；阶段 6 流内错误测试 | `v0.9.12` | 高 | 已验证双通道观测 |
...
| E-19 | `adk.GetMessage` 能从完整或流式 AgentEvent 提取消息，并在流式场景复制后拼接 stream；应用不保留事件时需关闭剩余副本 | 已验证 | [`adk/utils.go` `TypedGetMessage`](https://github.com/cloudwego/eino/blob/v0.9.12/adk/utils.go#L278)；`TestConsumeAgentEventMessageClosesRetainedStream` | Eino `v0.9.12` | 高 | 阶段 6 已验证 |
...
| E-21 | 自定义天气 Agent 的流式 ReAct、三类错误链和 per-run Callback 均可离线重复验证 | 已验证 | `examples/diagnosable-weather-agent/*_test.go`；`docs/learning/eino/failure-matrix.md`；仓库测试命令 | Eino `v0.9.12` + Go `1.26.3` | 高 | 阶段 6 回归通过 |
...
| E-24 | ChatModelAgent 首次运行冻结默认配置，但 ReAct Graph 在每次执行时创建并编译；流式迁移只把执行分支从 `Invoke` 切到 `Stream` | 已验证 | [`adk/chatmodel.go`](https://github.com/cloudwego/eino/blob/v0.9.12/adk/chatmodel.go#L1206) `buildMessageReActRunFunc`；阶段 6 scripted model 调用计数 | Eino `v0.9.12` | 高 | 阶段 6 已核对 |
...
| E-27 | EinoExt OpenAI 从调用级 model options 读取 ToolInfo，转换为 Chat Completions tools，并用同一 ctx 发起普通或流式 HTTP 请求 | 已验证 | EinoExt OpenAI `v0.1.13` `Generate`/`Stream`；ACL OpenAI `v0.1.17` `genRequest`、`Generate`/`Stream` 与 `CreateChatCompletionStream` | 精确模块版本 | 高 | 已纳入外部边界导航并完成在线流式冒烟 |
| E-28 | Runner 流式迁移后，两次模型调用都走 `ToolCallingChatModel.Stream`，分块回答经 `adk.GetMessage` 正确拼接，Tool 契约不变 | 已验证 | `TestWeatherAgentReAct`；`agent.go` 的 `EnableStreaming=true` | Eino `v0.9.12` + Go `1.26.3` | 高 | 阶段 6 已完成 |
| E-29 | 模型返回 StreamReader 后的中途错误由消息流消费返回，不会触发 ChatModel `OnError` | 已验证 | `TestWeatherAgentStreamError` 同时断言 ChatModel `started`、无 `failed` 且 `errors.Is` 命中原错误 | Eino `v0.9.12` + Go `1.26.3` | 高 | 阶段 6 已完成 |
| E-30 | 自定义 OpenAI 兼容代理可驱动流式天气 Agent 完成两次模型流调用、一次 Tool 调用和最终回答 | 已验证 | 2026-07-17 在线流式冒烟；退出码 0 | Eino `v0.9.12` + EinoExt OpenAI `v0.1.13` | 高 | 在线结果仅作冒烟，离线测试仍是回归基线 |
...
| 流式定向测试 | 两次模型 Stream、回答分块拼接、保留流副本关闭、流内错误与 Callback 边界全部通过 | 已执行，阶段 6 通过 |
| 阶段 6 执行 `go test ./...`、`go test -race ...`、`go vet ./...` | 全部通过，无竞态或 vet 报告 | 已执行，流式迁移回归通过 |
| 使用现有 `.env` 运行流式天气 Agent | 沙箱网络内首次失败；获准在宿主网络原样重跑后完成两次模型流调用、一次 Tool 调用和最终回答，退出码 0 | 已执行，阶段 6 在线冒烟通过 |
