...
- 示例：`examples/diagnosable-weather-agent/`，`RunnerConfig.EnableStreaming=false`。
...
|---|---|---|---|---|---|
| 正常 ReAct | scripted model 第一轮生成 `weather_lookup` ToolCall | Model -> ToolsNode -> Tool -> Tool message -> Model -> final `AgentEvent` | 两次模型调用；第二轮看到匹配 `ToolCallID` 的 Tool message | `Tool/weather_lookup` 有 `started`、`succeeded` 记录 | 已通过 |
...
2026-07-15 使用仓库现有 `.env` 中的自定义 OpenAI 兼容代理运行成功，未输出密钥或代理地址：

```bash
go run ./examples/diagnosable-weather-agent "北京天气怎么样？"
```

实际事件与 Callback 顺序包含：第一次 OpenAI ChatModel 调用、`weather_lookup` Tool 调用、第二次 OpenAI ChatModel 调用、ReAct Graph 完成和最终回答。Tool 返回北京晴天、28°C、湿度 35%，最终回答与该受控数据一致，进程退出码为 0。
...
| `go test ./examples/diagnosable-weather-agent/... -run 'TestWeather\|TestStatic\|TestNewWeather' -count=1` | 通过 |
...
阶段 4 的非流式正常路径和三类故障均已验证。流读取期错误不属于本阶段的非流式实现范围，将在阶段 6 把 `EnableStreaming` 作为唯一变量迁移后验证。
