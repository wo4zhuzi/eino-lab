...
本文从一次 `QualityGate.Review` 调用出发，追踪 Eino Compose `v0.9.12` 的数据、状态、分支和错误路径。
...
1. `NewQualityGate` 使用 `compose.NewGraph[ReviewRequest, ReviewResult]` 创建类型化 Graph，并通过 `WithGenLocalState` 注册 `gateState` 生成器。每次 `Runnable.Invoke` 都应得到一个新的状态实例。
2. `validate` 去除首尾空白；空内容直接返回 `ErrEmptyContent`。此时不调用 `Inspector`。
3. `inspect` 调用注入的 `Inspector`，校验评分范围，并通过 `compose.ProcessState` 增加尝试次数和审计记录。
4. `inspect` 的输出进入 `GraphBranch`。达标进入 `approve`；未达标且未达到 `MaxAttempts` 进入 `remediate`；达到上限进入 `manual`。
5. `remediate` 只做确定性内容补救，然后沿显式边回到 `inspect`。因此每次补救都必须重新检查，不能绕过质量门禁。
6. `approve` 或 `manual` 读取 Local State 的审计副本，构造 `ReviewResult` 并连接到 `END`。
7. `NewQualityGate` 在 Compile 时注入 `TopologySnapshotter`。它只复制名称、类型、节点、边和分支目标，不保留 Graph 节点实例。
