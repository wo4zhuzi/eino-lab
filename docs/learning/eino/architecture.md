...
默认使用可编程 ChatModel 和受控 WeatherProvider，避免真实模型的非确定性掩盖框架行为。在线模型是单独的显式实验，不进入默认 `go test ./...`。阶段 6 已把 Runner 从非流式单变量迁移为流式，并验证 MessageStream 消费、关闭和流内错误观测；模型、Tool 和业务逻辑未同时更换。

## 当前结论

阶段 2 至阶段 6 已生成并验证真实运行链路、故障矩阵和源码导航。上述架构结论限定于 Eino `v0.9.12` 与当前单 Agent、单 Tool 学习范围，不外推为生产就绪结论。
