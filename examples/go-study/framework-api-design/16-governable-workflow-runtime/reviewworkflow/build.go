...
func build(config Config, dependencies Dependencies) (*compose.Chain[Request, Result], error) {
	...
	chain.
		...
		AppendGraph(
			...
			compose.WithNodeName(nodeDecision),
...
func buildDecisionGraph(handlers *handlers) (*compose.Graph[reviewDraft, reviewDecision], error) {
	...
	if err := graph.AddLambdaNode(
		nodeApprove,
		compose.InvokableLambda(handlers.approve),
		compose.WithNodeName(nodeApprove),
	); err != nil {
		return nil, fmt.Errorf("添加节点 %s: %w", nodeApprove, err)
	}
	if err := graph.AddLambdaNode(
		nodeManual,
		compose.InvokableLambda(handlers.manual),
		compose.WithNodeName(nodeManual),
	); err != nil {
