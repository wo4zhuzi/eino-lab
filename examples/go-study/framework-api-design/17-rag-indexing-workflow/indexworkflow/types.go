...
var (
	...
	ErrEmptySourceURI       = errors.New("source_uri 不能为空")
	ErrSourceNotRegularFile = errors.New("数据源不是普通文件")
	ErrSourceTooLarge       = errors.New("数据源超过大小限制")
	ErrUnsupportedFormat    = errors.New("不支持的文件格式")
	ErrInvalidFileFormat    = errors.New("文件内容与格式不匹配")
	ErrNoParsedContent      = errors.New("解析结果没有有效内容")
...
// SourceInfo 是解析前完成校验和指纹计算的数据源快照。
