...
随后 `addLambdaNode` 执行这个 Option，把 Handler 保存进节点配置：
...
- `configuredNode.config.postHandler` 实际就是之前登记的 `saveQuestion`。
- `question` 是节点输出。
- `state` 是框架为本次运行创建的状态。
- 因此调用 `configuredNode.config.postHandler(...)` 等同于调用 `saveQuestion(...)`。
...
| `addLambdaNode(...)` 应用并保存 Option | `graph.AddLambdaNode(...)` 应用并保存节点配置 |
| `runGraph(...)` | 编译后 Runnable 的一次运行 |
| `state := &queryState{}` | `WithGenLocalState` 为一次运行创建状态 |
| `configuredNode.config.postHandler(...)` | Eino 调用 PostHandler 并注入节点输出和 Local State |
...
```text
节点最终输出："已校验：state 从哪里来？"
PostHandler 保存的状态："已校验：state 从哪里来？"
```
...
3. 阅读 `addLambdaNode`，确认 Option 把 `saveQuestion` 保存到节点配置。
4. 阅读 `runGraph`，找到真正调用 `saveQuestion` 和传入状态的位置。
