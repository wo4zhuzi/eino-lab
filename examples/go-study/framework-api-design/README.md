...
组合知识：可治理工作流、Eino Chain、文档 Loader 与 Parser、格式路由、稳定标识和错误链。

学习目标：建立从数据源检查、真实解析到 Chunk、Embedding、持久化、校验和发布的完整索引拓扑，并明确区分真实阶段与模拟阶段。

可运行示例：[17-rag-indexing-workflow](17-rag-indexing-workflow/README.md)。

通过标准：支持的本地文档能够进入标准化解析单元；非法格式立即失败；尚未实现的下游节点不会伪造成功结果。

### 18. 解耦文档摄取

组合知识：接口隔离、依赖注入、组件所有权、Eino Document 元数据和工作流版本演进。

学习目标：将通用 Loader、Parser、格式校验和远程数据源能力迁移到独立摄取组件，索引工作流只保留索引语义和编排职责。
