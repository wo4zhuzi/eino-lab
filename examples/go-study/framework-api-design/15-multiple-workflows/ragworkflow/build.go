...
func build(config Config, dependencies Dependencies) (*compose.Chain[Request, Result], error) {
	...
	chain.
		...
		AppendLambda(compose.InvokableLambda(handlers.generate), compose.WithNodeKey(nodeGenerate)).
...
func buildRetrievalGraph(handlers *handlers) (*compose.Graph[queryState, retrievalOutcome], error) {
	...
	if err := graph.AddLambdaNode(nodeRetrieve, compose.InvokableLambda(handlers.retrieve)); err != nil {
		return nil, fmt.Errorf("添加节点 %s: %w", nodeRetrieve, err)
	}
	if err := graph.AddLambdaNode(nodeRewrite, compose.InvokableLambda(handlers.rewrite)); err != nil {
		return nil, fmt.Errorf("添加节点 %s: %w", nodeRewrite, err)
	}
	if err := graph.AddLambdaNode(nodeEvidenceReady, compose.InvokableLambda(handlers.evidenceReady)); err != nil {
		return nil, fmt.Errorf("添加节点 %s: %w", nodeEvidenceReady, err)
	}
	if err := graph.AddLambdaNode(nodeNoEvidence, compose.InvokableLambda(handlers.noEvidence)); err != nil {
